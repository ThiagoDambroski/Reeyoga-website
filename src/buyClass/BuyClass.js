...
function BuyClass() {
  ...
  useEffect(() => {
    const interval1 = setInterval(() => {
      if (!userClicked) {
        setIndex((prevIndex) =>
          prevIndex + 1 > imageArray.length - 1 ? 0 : prevIndex + 1
        );
        setIndex2((prevIndex2) =>
          prevIndex2 - 1 < 0 ? imageArray2.length - 1 : prevIndex2 - 1
        );
      }
    }, 4000);
  ...
  const openWhatsApp = () => {
    ...
    const message = encodeURIComponent(
      "Ola, eu gostaria de marcar uma aula de yoga."
    );
  ...
  return (
    <div className="buy-class-page">
      <div className="buy-class-informations">
        ...
        <p>
          Descubra os benefícios transformadores do yôga e dê o primeiro passo
          em direção a uma vida mais saudável, equilibrada e harmoniosa. Eu
          estou pronta para guiá-lo em sua jornada de autodescoberta. bem-estar
          e conhecimento ancestral que você carrega dentro de si.
          <br />
          <br />
          Com as práticas milenares do yôga, você poderá fortalecer seu corpo,
          conduzir sua mente e encontrar um equilíbrio profundo que o ajudará a
          lidar com o estresse da vida moderna, claro com a sua permissão
          interna.
          <br />
          <br />
          Monteseu grupo e marque sua aula hoje e comece a investir em si mesmo,
          permitindo que o yôga o conduza a cultivar uma maior conexão consigo
          mesmo e com o mundo ao seu redor.
          <br />
          <br />
          Monte seu grupo , não importa o lugar nós podemos estar juntos e fazer
          um dia memorável e criar uma nova percepção de vida.
        </p>
        <div className="buy-classe-button-container">
          <button className="buy-class-button" onClick={() => openWhatsApp()}>
            Marque sua aula online{" "}
          </button>
        </div>
      </div>
      <div className="buy-class-photos">
        <div
          className="buy-class-carousel-container"
